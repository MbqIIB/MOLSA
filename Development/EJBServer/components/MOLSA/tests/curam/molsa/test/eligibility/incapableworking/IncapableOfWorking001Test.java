...
public class IncapableOfWorking001Test extends CERScenarioTestBase {
	...
	public IncapableOfWorking001Test(String arg0) {
		super(arg0);
	...
	protected void addIntakeApplicant(Application application)
		...
		createIntakeApplicant(application.getID(), RAFI_UNIQUE_NAME,
	...
	protected void addEvidence(CaseKey caseKey) throws AppException,
		...
		long childparticipantid = getParticipantRoleID(RAFI_UNIQUE_NAME).participantRoleID;
		long childcaseParticipantRoleID = getCaseParticipantRoleID(RAFI_UNIQUE_NAME).caseParticipantRoleID;
		long wifeparticipantid = getParticipantRoleID(FATHIMA_UNIQUE_NAME).participantRoleID;
		long wifecaseParticipantRoleID = getCaseParticipantRoleID(FATHIMA_UNIQUE_NAME).caseParticipantRoleID;
		...
		createHouseholdMemberEvidence(caseKey, childparticipantid,
				...
				RESIDENCY.YES);
		createHouseholdMemberEvidence(caseKey, wifeparticipantid,
				wifecaseParticipantRoleID, currentDate, CITIZENSHIPCODE.QATARI,
				RESIDENCY.YES);
		
		createBirthAndDeathEvidence(caseKey, participantid,
				caseParticipantRoleID, currentDate, getDate(1, 1, 1974));
		createBirthAndDeathEvidence(caseKey, childparticipantid,
				childcaseParticipantRoleID, currentDate, getDate(1, 1, 2000));
		createBirthAndDeathEvidence(caseKey, wifeparticipantid,
				wifecaseParticipantRoleID, currentDate, getDate(1, 1, 1984));
		...
		createMaritalStatusEvidence(caseKey,wifeparticipantid,
				wifecaseParticipantRoleID, currentDate, MARITALSTATUS.MARRIED);
		...
		createUnableToWorkEvidnece(caseKey,
				participantid,caseParticipantRoleID,
				getDate(1, 1, 2012),HANDICAPPEDUNABLETOWORK.UNABLETOWORK);
		
		createHouseholdRelationshipEvidence(caseKey,
				participantid,caseParticipantRoleID,
				wifecaseParticipantRoleID,getDate(1,1,2000),
				RELATIONSHIPTYPECODE.SPOUSE);
		createHouseholdRelationshipEvidence(caseKey,
				participantid,caseParticipantRoleID,
				childcaseParticipantRoleID,getDate(1,1,2000),
				RELATIONSHIPTYPECODE.PARENT);
		createHouseholdRelationshipEvidence(caseKey,
				wifeparticipantid,wifecaseParticipantRoleID,
				childcaseParticipantRoleID,getDate(1,1,2000),
				RELATIONSHIPTYPECODE.PARENT);
	...
	protected void setClaimantRegistrationDetails(
		...
		customerRegistrationDetails.dateOfBirth = getDate(1, 1, 1974);
