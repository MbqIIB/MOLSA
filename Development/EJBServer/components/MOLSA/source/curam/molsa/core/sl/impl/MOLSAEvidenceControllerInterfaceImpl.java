...
/**
 * Class implements OOTB hook point to temporary stop the cases when the user apply the evidence changes to
 * the integrated cases.
 */
public class MOLSAEvidenceControllerInterfaceImpl implements EvidenceControllerInterface.EvidenceActivationEvents {
  @Override
  /**
   ...
   *             Generic Exception Signature.
   */
  public void postActivation(EvidenceControllerInterface evidencecontrollerinterface, CaseKey casekey, EIEvidenceKeyList eievidencekeylist) throws AppException,
    ...
    CaseHeaderReadmultiDetails1List caseHeaderReadmultiDetails1List = caseHeaderObj.searchByIntegratedCaseID(paramCaseHeaderReadmultiKey);
    ...
    if(eievidencekeylist.dtls.size()==CuramConst.gkOne){
      ...
      if(evidenceKey.evidenceType.equals(CASEEVIDENCEEntry.ADDITIONALBENEFIT.getCode())){
    ...
    for (CaseHeaderReadmultiDetails1 caseHeaderReadmultiDetails1 : caseHeaderReadmultiDetails1List.dtls) {
      ...
      if (CASESTATUSEntry.ACTIVE.getCode().equals(caseHeaderReadmultiDetails1.statusCode) && !additionalBenefitEvidenceType) {
        ...
        paramCaseHeaderDtls = cachedCaseHeaderObj.read(paramCaseHeaderKey);
        paramCaseHeaderDtls.statusCode = CASESTATUSEntry.SUSPENDED.getCode();
        cachedCaseHeaderObj.modify(paramCaseHeaderKey, paramCaseHeaderDtls);

        currentCaseStatusKey.caseID = caseHeaderReadmultiDetails1.caseID;
        currentCaseStatusKey.nullDate = Date.kZeroDate;
        CaseStatusDtls caseStatusDtls = caseStatusObj.readCurrentStatusByCaseID1(currentCaseStatusKey);
        ...
        CaseStatusDtls caseStatusDtls1 = caseStatusObj.readCurrentStatusByCaseID1(currentCaseStatusKey);
        ...
        caseStatusDtls1.endDateTime = TransactionInfo.getSystemDateTime();
