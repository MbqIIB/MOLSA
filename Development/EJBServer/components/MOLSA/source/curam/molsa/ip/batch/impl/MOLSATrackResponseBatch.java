...
import curam.molsa.ip.batch.fact.MOLSATrackResponseBatchStreamFactory;
 ...
 * 
 */
public class MOLSATrackResponseBatch extends curam.molsa.ip.batch.base.MOLSATrackResponseBatch {
  ...
  public MOLSATrackResponseBatch() {
   
    String chunkSize = Configuration.getProperty(EnvVars.ENV_MOLSATRACKRESPONSE_CHUNK_SIZE);
    ...
    this.kDontRunStream = Configuration.getBooleanProperty(EnvVars.ENV_MOLSATRACKRESPONSE_DONT_RUN_STREAM);

    String chunkKeyReadWait = Configuration.getProperty(EnvVars.ENV_MOLSATRACKRESPONSE_CHUNK_KEY_WAIT_INTERVAL);
    ...
    String unProcessedChunkReadWait = Configuration.getProperty(EnvVars.ENV_MOLSATRACKRESPONSE_UNPROCESSED_CHUNK_WAIT_INTERVAL);
    ...
    this.kProcessUnProcessedChunks = Configuration.getBooleanProperty(EnvVars.ENV_MOLSATRACKRESPONSE_PROCESS_UNPROCESSED_CHUNKS);
  }
  ...
  public void sendBatchReport(String instanceID, BatchProcessDtls batchProcessDtls, BatchProcessChunkDtlsList processedBatchProcessChunkDtlsList,
    ...
    if (totalNumberOfUnprocessedChunks > 0) {
      ...
      final AppException errChunksSkippedText = new AppException(curam.message.BPOMOLSATRACKRESPONSEBATCH.ERR_CHUNKS_SKIPPED);
    ...
    final AppException infTotalCasesText = new AppException(curam.message.BPOMOLSATRACKRESPONSEBATCH.INF_CASE_RECORDS_PROCESSED);
    ...
    if (totalNumberOfCasesSkipped > 0) {
      ...
      final AppException infTotalSkippedCasesText = new AppException(curam.message.BPOMOLSATRACKRESPONSEBATCH.INF_CASE_RECORDS_SKIPPED);
    ...
    curamBatchObj.setEmailSubject(curam.message.BPOMOLSATRACKRESPONSEBATCH.INF_CASE_RECORDS_PROCESSED);
  ...
  public void process() throws AppException, InformationalException {
    ...
    curam.molsa.ip.batch.intf.MOLSATrackResponseBatchStream molsaTrackResponseBatchStream = MOLSATrackResponseBatchStreamFactory.newInstance();

    MOLSATrackResponseBatchStreamWrapper molsaTrackResponseBatchStreamWrapper = new MOLSATrackResponseBatchStreamWrapper(molsaTrackResponseBatchStream);

    MOLSATrackResponseBatchWrapper molsaTrackResponseBatchWrapper = new MOLSATrackResponseBatchWrapper(this);

    SecurityImplementationFactory.register();

    batchStreamHelper.setStartTime();

    String instanceID = BATCHPROCESSNAME.MOLSA_TRACKRESPONSE;

    MOLSAInformationRequest molsaInformationRequest = MOLSAInformationRequestFactory.newInstance();
    MOLSAInformationRequestDtlsList molsaInformationRequestDtlsList = molsaInformationRequest.nkreadmulti();

    BatchProcessingIDList batchProcessingIDList = new BatchProcessingIDList();
    // loop through to set the batch processing details
    for (MOLSAInformationRequestDtls molsaInformationRequestDtls : molsaInformationRequestDtlsList.dtls) {
      BatchProcessingID batchProcessingID = new BatchProcessingID();
      batchProcessingID.recordID = molsaInformationRequestDtls.informationRequestID;
    ...
    batchStreamHelper.runChunkMain(instanceID, null, molsaTrackResponseBatchWrapper, batchProcessingIDList, chunkMainParameters, molsaTrackResponseBatchStreamWrapper);
  }
}
