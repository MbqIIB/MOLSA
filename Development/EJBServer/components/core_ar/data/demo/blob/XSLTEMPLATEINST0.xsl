  ...
  <xsl:template match="STRUCT">
    <fo:root>
      ...
      <fo:page-sequence master-reference="only">
        ...
        <!-- END NON-TRANSLATABLE -->
		<fo:block xsl:use-attribute-sets="Normal_1">اشعار التسجيل</fo:block>
          ...
          <fo:block xsl:use-attribute-sets="Normal_8">
            نريد اعلامك بأنه قد تم تسجيلك باعتبارك
     <xsl:apply-templates select="FIELD[FNAME='concernRoleTypeDesc']" />
            في
     <xsl:apply-templates select="FIELD[FNAME='organisationName']" />.
          </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_8">
            وقم تم تخصيص رقم المرجع التالي
     <xsl:apply-templates select="FIELD[FNAME='alternateID']" />
            لك ويجب أن يتم استخدامه في كل الاتصالات مع المؤسسة.
          </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_9">مع تحياتي،</fo:block>
