...
<xsl:text>
    ...
    <xsl:if test="../INFO/HAS_FILE_EDIT">
      <xsl:apply-templates
        select="ancestor::PAGE/DISPLAY/descendant::WIDGET[@TYPE = 'FILE_EDIT']"
        mode="file-edit-loop-back"/>
    </xsl:if>
<xsl:text>
      }
...
<xsl:text>if (!action.equals(LOOP)) {
  ...
  <xsl:template match="WIDGET" mode="file-edit-loop-back">
    <xsl:text>
        if (hasFileEditWidget(rh)) {
          action = LOOP;
        }</xsl:text>
  </xsl:template>
