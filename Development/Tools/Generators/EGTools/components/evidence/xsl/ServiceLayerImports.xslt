...
<xsl:element name="imports">
  ...
  <xsl:if test="(Relationships/@association='Yes' and count(Relationships/Association[@from!=''])>0) or
                 $casePartRoleIDattr='' or count(Relationships/MandatoryParents/Parent)&gt;0">
    <xsl:element name="import">
  ...
  <!-- BEGIN, CR000, PADDY -->
  <xsl:if test="((count(Relationships/Parent)&gt;0 or count(Relationships/MandatoryParents/Parent)) and @relateEvidenceParticipantID='') or count(UserInterfaceLayer/Cluster[@view='Yes']/Field[@metatype=$metatypeParentCaseParticipant]) &gt; 0">
    <xsl:element name="import">
