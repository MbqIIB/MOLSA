...
<xsl:template name="ResolveRelatedCaseParticipant">

  <xsl:param name="prefix"/>
  <xsl:param name="path"/>
  <xsl:param name="UIName"/>
  
  <xsl:param name="relatedCaseParticipantName" />
...
<PAGE  PAGE_ID="{$UIName}">
  ...
  <JSP_SCRIPTLET>    
      ...
      // Retrieve the related caseParticipantRoleID
      ...
      if (caseParticipantRoleID.equals("0")) {

        url += "<xsl:value-of select="$prefix"/>_create<xsl:value-of select="$relatedCaseParticipantName"/>CaseParticipantPage.do?" + queryBuffer;

      } else {

        url += "<xsl:value-of select="$resolveParticipantHome"/>Page.do?caseParticipantRoleID="
