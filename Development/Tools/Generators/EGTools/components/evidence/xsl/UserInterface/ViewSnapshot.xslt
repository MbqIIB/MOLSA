...
<xsl:stylesheet extension-element-prefixes="redirect xalan" xmlns:redirect="org.apache.xalan.xslt.extensions.Redirect" version="1.0" xmlns:xalan="http://xml.apache.org/xslt" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="ViewSnapshot">
    
    <xsl:param name="path"/>
    <xsl:param name="UIName"/>
    <xsl:param name="VIMName"/>
    ...
    <xsl:variable name="viewContentUIName"><xsl:value-of select="$VIMName"/></xsl:variable>
...
<xsl:call-template name="printXMLCopyright">
  <xsl:with-param name="date" select="$date"/>
    ...
    <!-- BEGIN, PADDY -->
        <xsl:call-template name="write-all-locales-view-snapshot-properties">   
  ...
  <!-- BEGIN, PADDY -->
    ...
    <!--iterate through each token, generating each element-->
      <xsl:template name="write-all-locales-view-snapshot-properties">
         ...
         <xsl:if test="$locales">               
           ...
           <xsl:choose>
             ...
             <xsl:when test="contains($locales,',')">
               ...
               <xsl:call-template name="write-view-snapshot-properties">
      ...
      <xsl:template name="write-view-snapshot-properties">
