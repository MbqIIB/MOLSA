...
<xsl:template name="EmploymentModifyUI">
...
<PAGE_TITLE>
  <CONNECT>
    <SOURCE PROPERTY="PageTitle.StaticText1" NAME="TEXT"/>
  </CONNECT>
  <CONNECT>
...
<ACTION_CONTROL TYPE="SUBMIT" LABEL="ActionControl.Label.Save" IMAGE="SaveButton">
  <LINK SAVE_LINK="false" PAGE_ID="{$readEmploymentUI}" DISMISS_MODAL="false">
    <CONNECT>
...
<INCLUDE FILE_NAME="Person_modifyEmploymentView.vim"/>
  ...
  <xsl:call-template name="write-all-locales-EmploymentModifyUI-properties">
...
<!--iterate through each token, generating each element-->
  <xsl:template name="write-all-locales-EmploymentModifyUI-properties">
    ...
    <xsl:if test="$locales">
      ...
      <xsl:choose>
        ...
        <xsl:when test="contains($locales,',')">
          ...
          <xsl:call-template name="write-EmploymentModifyUI-properties">
  ...
  <xsl:template name="write-EmploymentModifyUI-properties">
...
<xsl:call-template name="callGenerateProperties">
  <xsl:with-param name="propertyNode" select="$employmentProperties/Help.PageDescription.ModifyEmployment.PageDescription"/>
  <xsl:with-param name="evidenceNode" select="&apos;&apos;"/>
  <xsl:with-param name="altPropertyName">Help.PageDescription</xsl:with-param>
</xsl:call-template>

<xsl:call-template name="callGenerateProperties">
  <xsl:with-param name="propertyNode" select="$employmentProperties/Page.Title.Modify.Employment "/>
  <xsl:with-param name="evidenceNode" select="&apos;&apos;"/>
