...
<xsl:template name="ResolveParentEvidence">
...
<PAGE  PAGE_ID="{$UIName}">
  <JSP_SCRIPTLET>
      ...
      if (evidenceType.equals("<xsl:value-of select="$ParentType"/>")) {
        url += "<xsl:value-of select="$prefix"/>_list<xsl:value-of select="@name"/><xsl:value-of select="$caseType"/>Page.do?" + queryBuffer;
      }
      ...
      if (evidenceType.equals("<xsl:value-of select="$ParentType"/>")) {
        url += "<xsl:value-of select="$prefix"/>_list<xsl:value-of select="@from"/><xsl:value-of select="$caseType"/>Page.do?" + queryBuffer;
      }
