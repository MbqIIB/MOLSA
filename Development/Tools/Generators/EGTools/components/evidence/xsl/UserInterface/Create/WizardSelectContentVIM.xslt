...
<xsl:value-of select="$entityElement/UserInterfaceLayer/WizardSelectDescription[@type='PREASSOC']/@description"/>=<xsl:value-of select="$entityElement/UserInterfaceLayer/WizardSelectDescription[@type='PREASSOC']/Label[@locale=$locale]/@value"/>
		  ...
		  </xsl:call-template></xsl:variable><xsl:text> </xsl:text><xsl:value-of select="$preAssocLabel"/>,</xsl:for-each></xsl:variable>
Wizard.SelectPREASSOC.Description=<xsl:value-of select="$start"/><xsl:value-of select="substring($propValue,0,string-length($propValue))"/><xsl:text> </xsl:text><xsl:value-of select="$end"/>
