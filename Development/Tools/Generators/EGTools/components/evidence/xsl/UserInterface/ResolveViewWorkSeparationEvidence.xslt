...
<xsl:template name="ResolveViewWorkSeparation">
...
<PAGE PAGE_ID="{$prefix}_resolveViewWorkSeparationEvidence">
<JSP_SCRIPTLET>
      ...
      String evidenceID =
        request.getParameter("evidenceID");
      ...
      if (parEvType.equals("EETE8")) {
        url += "<xsl:value-of select="$prefix"/>_viewWorkSeparationEvidencePage.do?" + queryBuffer;
      }
  
      

      if (parEvType.equals("EETE7")) {
        url += "<xsl:value-of select="$prefix"/>_viewWorkSeparationEvidenceForSelfEmploymentPage.do?" + queryBuffer;
      }
