...
<!--  <xsl:include href="JavaID2CodeValue.xslt" />-->
  ...
  <xsl:template name="ResolveEvidenceObject">
    ...
    <xsl:variable name="pagename"><xsl:value-of select="$prefix"/>_resolve<xsl:value-of select="$caseType"/>Object</xsl:variable>
...
<PAGE PAGE_ID="{$pagename}">
  <JSP_SCRIPTLET>
    ...
    <xsl:for-each select="//EvidenceEntities/EvidenceEntity">
      ...
      url += "<xsl:value-of select="$prefix"/>_view<xsl:value-of select="@name"/><xsl:value-of select="$caseType"/>Object" 
