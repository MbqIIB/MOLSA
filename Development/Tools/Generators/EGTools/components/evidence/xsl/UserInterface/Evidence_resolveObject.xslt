...
>
  ...
  <xsl:template match="products">
    ...
    <xsl:variable name="pagename">Evidence_resolveObject</xsl:variable>
...
<PAGE PAGE_ID="{$pagename}">
  <JSP_SCRIPTLET>    
    ...
    if (pd.equals("true")) {
      ...
      url = context + "Evidence_resolveParticipantObjectPage.do?successionID=" + successionID + "&amp;";
      ...
      <xsl:for-each select="//products/product[not(substring-after(@clientDir,'webclient/components/')='custom')]">
        ...
        url = context + "<xsl:value-of select="@prefix"/>_resolve" + caseTypeValue + "ObjectPage.do?";
      }
      </xsl:for-each>
