...
>
  ...
  <xsl:template match="EvidenceEntity[UserInterfaceLayer]">
    ...
    <!-- Top level entities need no wizards unless they have core employments -->
    <xsl:if test="$isTopLevel='false' or $hasCoreEmployment='true'">
      ...
      <xsl:variable name="selectAllWizardCreatePageName"><xsl:call-template name="Utilities-getCreatePageName-WizardSelectAll-ContentPage">
        <xsl:with-param name="entityElement" select="$entityElement"/><xsl:with-param name="caseType" select="$caseType"></xsl:with-param>
      </xsl:call-template></xsl:variable>
