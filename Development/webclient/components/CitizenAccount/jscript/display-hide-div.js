...
var recommendations = {
	...
	hideServiceDescriptions: function() {
		...
		var mapNode = dojo.byId('curam_map_div_id');
		
		if(mapNode) {
		
			initialMapWidth = dojo.marginBox(mapNode).w;
...
/*
 * MIA, hack for defect: GUM-3464
 */
(function() {
	var probCtx = "o3ctx=1048576",
		l = location.toString();

	if (l.indexOf("/CitizenAccount_viewRecommendationsPage.do?") > 0 && l.indexOf(probCtx) !== -1) {
		l = l.replace(probCtx, "o3ctx=4096");
		window.location = l;
	}
})();