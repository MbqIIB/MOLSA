...
var linkDialog = function( editor, linkMode )
{
	...
	var urllinkPlugin = editor.plugins['urllink'],
		innermostElement,
		isBookmark = false;
	...
	var parseLink = function( element )
	{
		...
		if (anchorMatch = href.match(anchorRegex)){		//editing an existing anchor link
		...
		if (linkMode == 'bookmark' || isBookmark)
			this.parts.title.setText(editor.lang.ibm.anchor.title);
		else
			this.parts.title.setText(editor.lang.ibm.urllink.title);
    ...
    return {
        title: editor.lang.ibm.urllink.title, //setting urllink.title as the default - this will get updated in parseLink as part of onShow()
		...
		[
			{
				...
				[
					{		//bookmark fields
						...
						'default' : '',
						required : true,
						label : editor.lang.ibm.anchor.linkTo,
						style : 'width: 100%;',
						items :
						[
							[ '' ]
					...
					{		//URL fields
						...
						validate : function()
						{
							...
							var func = CKEDITOR.dialog.validate.notEmpty(editor.lang.ibm.urllink.nourl);
					...
					},
					{
						...
						label: editor.lang.ibm.urllink.linkText,
					...
					},
					{
						type: 'checkbox',
						id: 'chkNewWindow',
						label: editor.lang.ibm.urllink.openinnew,
						'default': false,
						setup: function(data) {
							if (linkMode != 'urllink' || isBookmark)
								this.getElement().hide();
