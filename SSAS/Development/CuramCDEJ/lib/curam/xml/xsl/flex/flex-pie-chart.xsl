...
<xsl:template match="CHART|PIE_CHART">
...
<!--     <xsl:value-of select="concat('ident=', $ident,
 ...
 -->                                 
   ...
   <xsl:variable name="chart-type">
       <xsl:value-of select="concat($static-content-server-url,'/CDEJ/flex/piechart.swf')"/>
   </xsl:variable>
  ...
  <script type="text/javascript">
    ...
    <object id="{$ident}" classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='https://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0' width='100%'>
      <param name='src' value="{$chart-type}"/>
      <param name='flashVars' value="{$vars-to-pass}"/>
      <embed name='{$ident}' src='{$chart-type}' pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' height='100%' width='100%' flashVars="{$vars-to-pass}"/>
    </object>
</xsl:template>
