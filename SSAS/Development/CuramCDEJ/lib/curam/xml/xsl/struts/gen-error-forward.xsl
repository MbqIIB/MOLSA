...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <global-forwards>
      ...
      <xsl:for-each select="ERROR_PAGE[@TYPE]">
        ...
        <xsl:call-template name="exception-to-error-page">
          ...
          <xsl:with-param name="path-prefix"><xsl:call-template name="first-two-chars"><xsl:with-param name="string" select="@PAGE_ID"/></xsl:call-template></xsl:with-param>
  ...
  <xsl:template name="handle-default-error-page">
    ...
    <xsl:call-template name="exception-to-error-page">
      <xsl:with-param name="name" select="concat('do', $type ,'Error')"/>
      <xsl:with-param name="path-prefix"><xsl:call-template name="first-two-chars"><xsl:with-param name="string" select="$defaultErrorPage"/></xsl:call-template></xsl:with-param>
  ...
  <xsl:template name="first-two-chars">
    <xsl:param name="string"/>
    <xsl:value-of select="translate(substring($string, 1, 2),
