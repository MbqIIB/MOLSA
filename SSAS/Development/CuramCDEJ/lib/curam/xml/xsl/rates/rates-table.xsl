...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <table xsl:use-attribute-sets="rates-table">
        ...
        <xsl:if test="number($num-sub-cols) &gt; 0">
          <tr xsl:use-attribute-sets="header">
            ...
            <td class="sub-column-header" colspan="2" style="width:360px;"><span class="border-span"><xsl:text>&#160;</xsl:text></span></td>      
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <td class="empty-cell"><xsl:text>&#160;</xsl:text></td>
              </xsl:otherwise>
            </xsl:choose>
            </xsl:for-each>
          </tr>
      ...
      <xsl:when test="number($num-sub-cols) &gt;= 1">
        <xsl:choose>
          ...
          <xsl:when test="number($num-rows) &gt; 0 and
            ...
            <td xsl:use-attribute-sets="span-two-cols">
              <span class="border-span"><xsl:call-template name="output-top-left-buttons" /></span>
            </td>
          </xsl:when>
          <xsl:otherwise>
            <td xsl:use-attribute-sets="first-column-span-two-cols">
              <span class="border-span"><xsl:call-template name="output-top-left-buttons" /></span>
            </td>
          </xsl:otherwise>
        </xsl:choose>
  ...
  <xsl:template name="do-column">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$num-sub-cols &gt;= 1 and number($num-sub-cols-current-col) = 0">
        <td xsl:use-attribute-sets="span-two-rows">
  ...
  <xsl:template match="SUB_COLUMN" mode="do-sub-column">
    <td xsl:use-attribute-sets="sub-column-header">
  ...
  <xsl:template name="do-row-buttons">
    <xsl:choose>
      <xsl:when test="not($read-only)">      
        <a href="#" onclick="dojo.stopEvent(arguments[0]);require(['curam/util/UimDialog'], function() {{ curam.util.UimDialog.openUrl(
  ...
  <xsl:template name="do-column-buttons">
    <xsl:choose>
      <xsl:when test="not($read-only)">
        <a href="#" onclick="dojo.stopEvent(arguments[0]);require(['curam/util/UimDialog'], function() {{ curam.util.UimDialog.openUrl('{concat($config/RATES_TABLE_CONFIG/MANAGE_COLUMN_PAGE,
  ...
  <xsl:template name="output-button">
    ...
    <a href="#" onclick="dojo.stopEvent(arguments[0]);require(['curam/util/UimDialog'], function() {{ curam.util.UimDialog.openUrl('{concat($href,
