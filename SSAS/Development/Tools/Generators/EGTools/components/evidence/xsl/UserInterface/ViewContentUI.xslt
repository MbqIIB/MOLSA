...
Field.Label.Associated<xsl:value-of select="@from"/>.Help=<xsl:value-of select="@comments"/><xsl:text>&#xa;</xsl:text>
  ...
  <xsl:template name="ViewContentUI-WriteClusters">
    ...
    <xsl:for-each select="$Clusters">
      ...
      <xsl:if test="(($viewVersion=$businessObjectViewVersion and count(Field[not(@metatype) or @metatype!=$metatypeParentCaseParticipant])>0) or ($viewVersion!=$businessObjectViewVersion and count(Field)>0) or count(SkipField)&gt;0 or count(EvidenceHeader)&gt;0 or count(LinkField)&gt;0) or count(association)>0">
    ...
    <xsl:when test="@metatype=$metatypeCaseParticipantSearch or @metatype=$metatypeEmployerCaseParticipant  or @metatype=$metatypeDisplayCaseParticipant or (@createCaseParticipant='Yes' and @name!='')">
