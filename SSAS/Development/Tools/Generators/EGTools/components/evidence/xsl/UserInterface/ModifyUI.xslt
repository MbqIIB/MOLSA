...
<xsl:template name="ModifyUI">
  ...
  <xsl:variable name="listUIName"><xsl:value-of select="$prefix"/>_resolve<xsl:value-of select="$capNameCaseType"/>List</xsl:variable>
...
<xsl:if test="$modifiable='Yes'">
  ...
  <INCLUDE FILE_NAME="{$modifyContentUIName}.vim"/>
...
<!-- BEGIN, PADDY -->
  ...
  <!--iterate through each token, generating each element-->
    <xsl:template name="write-all-locales-modify-properties">
       ...
       <xsl:if test="$locales">               
         ...
         <xsl:choose>
           ...
           <xsl:when test="contains($locales,',')">
             ...
             <xsl:call-template name="write-modify-properties">
    ...
    <xsl:template name="write-modify-properties">
