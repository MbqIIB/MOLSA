...
<xsl:stylesheet
    ...
    <xsl:template name="ResolveViewEvidenceForProduct">
        ...
        <redirect:write select="concat($filepath, '.uim')">
            ...
            <JSP_SCRIPTLET>
                ...
                if(integratedCaseID.compareTo("0")!=0){pageName="<xsl:value-of select="$prefix"/>_view<xsl:value-of select="$capName"/>ProductEvidence";}
                else {pageName="<xsl:value-of select="$prefix"/>_view<xsl:value-of select="$capName"/>Evidence";}
