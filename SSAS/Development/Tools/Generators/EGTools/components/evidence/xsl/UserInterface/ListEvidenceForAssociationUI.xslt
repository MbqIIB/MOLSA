...
<xsl:template name="ListEvidenceForAssociationUI">
  ...
  <xsl:variable name="listUIName">
    ...
    <xsl:choose>
      <xsl:when test="count($ParentEvidenceEntity/Relationships/Parent)>1"><xsl:value-of select="$prefix"/>_resolve<xsl:value-of select="$firstParentName"/><xsl:value-of select="$caseType"/>List</xsl:when>  
      <xsl:when test="count($ParentEvidenceEntity/Relationships/Association[@from!=''])>1"><xsl:value-of select="$prefix"/>_resolve<xsl:value-of select="$firstParentName"/><xsl:value-of select="$caseType"/>List</xsl:when>  
      ...
      <xsl:otherwise><xsl:value-of select="$prefix"/>_list<xsl:value-of select="$capName"/><xsl:value-of select="$caseType"/></xsl:otherwise>
    </xsl:choose>    
  </xsl:variable>

  <xsl:variable name="viewUIName">
    <xsl:choose>
      <xsl:when test="$fromCreateParentInd='Yes'"><xsl:value-of select="$prefix"/>_view<xsl:value-of select="$capName"/><xsl:value-of select="$caseType"/>FromAssociationList_fromCreateParent</xsl:when>  
  ...
  <xsl:variable name="createUIName">
    <xsl:choose>
      <xsl:when test="$fromCreateParentInd='Yes'"><xsl:value-of select="$prefix"/>_create<xsl:value-of select="$capName"/><xsl:value-of select="$caseType"/>FromAssociationList_fromCreateParent</xsl:when>  
  ...
  <xsl:variable name="confirmCreateAssociationUIName">
    <xsl:choose>
      <xsl:when test="$fromCreateParentInd='Yes'"><xsl:value-of select="$prefix"/>_confirmCreate<xsl:value-of select="$capName"/><xsl:value-of select="$caseType"/>Association_fromCreateParent</xsl:when>  
      <xsl:otherwise><xsl:value-of select="$prefix"/>_confirmCreate<xsl:value-of select="$capName"/><xsl:value-of select="$caseType"/>Association</xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
...
>
  ...
  <!--<CONNECT>
    ...
    <TARGET
      NAME="DISPLAY"
      PROPERTY="key$key$parentEvidenceID"
    />
  </CONNECT>
  ...
  <CONNECT>
    ...
    <TARGET
      NAME="DISPLAY"
      PROPERTY="key$key$parentEvidenceType"
    />
  </CONNECT>-->
...
<!-- BEGIN, PADDY -->
  <xsl:call-template name="write-all-locales-list-association-properties">   
...
<!-- BEGIN, PADDY -->
  ...
  <!--iterate through each token, generating each element-->
    <xsl:template name="write-all-locales-list-association-properties">
       ...
       <xsl:if test="$locales">               
         ...
         <xsl:choose>
           ...
           <xsl:when test="contains($locales,',')">
             ...
             <xsl:call-template name="write-list-association-properties">
    ...
    <xsl:template name="write-list-association-properties">
    ...
    <xsl:if test="count(//EvidenceEntities/Properties[@locale=$locale]/General)&gt;0">
     <redirect:write select="concat($filepath, $locale, '.properties')">
        <!-- NOTE Old implementation of properties got entity name from displayName which was not available. This may
          have been a bug as the context node used in this style sheet is the association node, not the entire entity  and
          therefore displayName would never be available like this. This needs to be properly tested i.e check labeling on
        page at runtime etc-->
     
    <xsl:variable name="generalProperties" select="//EvidenceEntities/Properties[@locale=$locale]/General"/>
    
    <xsl:if test="count($generalProperties/Help.PageDescription.List.AssociatedEvidence2)&gt;0">
<xsl:call-template name="callGenerateProperties">
      <xsl:with-param name="propertyNode" select="$generalProperties/Help.PageDescription.List.AssociatedEvidence2"/>
