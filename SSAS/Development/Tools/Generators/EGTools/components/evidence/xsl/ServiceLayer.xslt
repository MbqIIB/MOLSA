...
<!-- BEGIN, 25/02/2008, CD -->
  <xsl:for-each select="UserInterfaceLayer/Cluster/Field[@mandatory='Yes']">
    ...
    //
    if(dtls.<xsl:value-of select="$aggregation"/>.<xsl:value-of select="@columnName"/> == new <xsl:value-of select="$struct"/>().<xsl:value-of select="@columnName"/>) {
<!-- END, 25/02/2008, CD -->
      AppException appException = new AppException(
  ...
  public SearchCaseParticipantDetails getCaseParticipantRoleID(EIEvidenceKey key)
    ...
    if (parentList.dtls.size() > 0) {
      ...
      SearchCaseParticipantDetails valueFrom<xsl:value-of select="@name"/> =
        (<xsl:value-of select="$javaEvidenceCodePath"/>.service.fact.<xsl:value-of select="@name"/>Factory
        .newInstance().getCaseParticipantRoleID(eiEvidenceKey));

      if (valueFrom<xsl:value-of select="@name"/>.caseParticipantRoleID != 0) {
        return valueFrom<xsl:value-of select="@name"/>;
      }
        </xsl:for-each>
    }
  ...
  public EvidenceComparisonDtls getComparisonData(EvidenceCaseKey key)
    ...
    <xsl:value-of select="$ReadEvidenceDetails"/><xsl:text> </xsl:text><xsl:value-of select="$readEvidenceDetails"/> = 
      read<xsl:value-of select="$EntityName"/>Evidence(key);
    <!-- BEGIN, CR00120297, CD -->  
    ...
    for (int i=0; i&lt;names.length &amp;&amp; i&lt;valueObjects.length; i++) {
