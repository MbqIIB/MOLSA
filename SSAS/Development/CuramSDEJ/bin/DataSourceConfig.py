...
def setupProvider (  ):
        ...
        for variable in variableList:
                ...
                if (cmp("ZOSRRS", dbtype) == 0):
                        if (cmp("DB2UNIVERSAL_JDBC_DRIVER_PATH", existingvariable) == 0):
                                valattr1 = ["value", DB2DIR+"/classes"]
                                attrs = [valattr1]
                                AdminConfig.modify(variable, attrs )
                        #endIf
                        if (cmp("DB2UNIVERSAL_JDBC_DRIVER_NATIVEPATH", existingvariable) == 0):
...
dbtype = sys.argv[2]
contAlias = sys.argv[3]
CURAMSDEJ = sys.argv[4]
DB2JCC_LICENSE_CISUZ_JAR = sys.argv[5]
dbname = sys.argv[6]
dbServiceName = sys.argv[7]
dbport = sys.argv[8]
dbmachine = sys.argv[9]
dbdrivertype = sys.argv[10]
platform = sys.argv[11]
DB2DIR = sys.argv[12]
purescale = sys.argv[13]
if (len(sys.argv)>14):
        # This argument is optional for when DB2 Type 2 is being used on z/OS.
        dbzosjccpropfile = sys.argv[14]
else:
        dbzosjccpropfile = ""
#endIf
...
if (cmp("DB2", dbtype) == 0):
        ...
        dstemplateName = "DB2 Universal JDBC Driver XA DataSource"
...
if (cmp("ZOSRRS", dbtype) == 0):
        templateName = "DB2 Universal JDBC Driver Provider"
        dstemplateName = "DB2 Universal JDBC Driver DataSource"
        datasourceHelperClassname = "com.ibm.websphere.rsadapter.DB2UniversalDataStoreHelper"
