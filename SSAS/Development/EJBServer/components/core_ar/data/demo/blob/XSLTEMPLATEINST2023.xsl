      ...
      <fo:page-sequence master-reference="only">
        ...
        <fo:flow flow-name="xsl-region-body" font-family="WT Sans ME">
          ...
          <fo:block xsl:use-attribute-sets="Normal_4">
            <xsl:apply-templates select="FIELD[FNAME='supplierConcernRoleAddress']" />
          </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_7"> </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_5"> </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_6">
            عزيزي <xsl:apply-templates select="FIELD[FNAME='supplierConcernRoleName']" />
          </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_7"> </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_6">
            نريد اعلامك أن <xsl:apply-templates select="FIELD[FNAME='productType']" />
            الحالة برقم المرجع،
              <xsl:apply-templates select="FIELD[FNAME='caseReference']" />,
            الى
            <xsl:apply-templates select="FIELD[FNAME='concernRoleName']" />
            وقد تم اغلاق (كود التعريف
