    ...
    <fo:root>
      ...
      <fo:page-sequence master-reference="only">
        ...
        <fo:flow flow-name="xsl-region-body" font-family="WT Sans ME">
          ...
          <!-- Dear .. -->
          <fo:block xsl:use-attribute-sets="Normal_1">
            عزيزي <xsl:apply-templates select="./FIELD[FNAME='externalPartyDtls']/STRUCT[SNAME='ConcernRoleNameAndAlternateID']/FIELD[FNAME='concernRoleName']" />
            ,
          </fo:block>
          ...
          <!-- Main body -->
          <fo:block xsl:use-attribute-sets="Normal_1">
            تريد اعلامك بأنه قد تم اعادة الفتح لك كطرف خارجي مع هيئتنا بموجب رقم المرجع،
...
<xsl:apply-templates select="./FIELD[FNAME='externalPartyDtls']/STRUCT[SNAME='ConcernRoleNameAndAlternateID']/FIELD[FNAME='primaryAlternateID']" />
            يطبق بدءا من
<xsl:apply-templates select="./FIELD[FNAME='modifyDate']" />.
          </fo:block>
          <fo:block linefeed-treatment="preserve" font-size="12.0pt" margin-left="0.0pt" space-after="0.0pt" space-before="0.0pt" text-align="start" text-indent="0.0pt"> </fo:block>
          <!-- Closing block -->
          <fo:block xsl:use-attribute-sets="Normal_1">مع تحياتي،</fo:block>
          <fo:block linefeed-treatment="preserve" font-size="12.0pt" margin-left="0.0pt" space-after="0.0pt" space-before="0.0pt" text-align="start" text-indent="0.0pt"> </fo:block>
          <fo:block xsl:use-attribute-sets="Normal_1">
