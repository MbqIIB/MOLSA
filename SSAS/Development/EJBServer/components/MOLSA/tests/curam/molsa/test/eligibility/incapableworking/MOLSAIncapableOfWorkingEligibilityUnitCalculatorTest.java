...
import curam.creole.ruleclass.IncapableOfWorkingEligibilityAndEntitlementRuleSet.impl.IncapableOfWorkingCPRCalculator;
import curam.creole.ruleclass.IncapableOfWorkingEligibilityAndEntitlementRuleSet.impl.IncapableOfWorkingCPRCalculator_Factory;
import curam.creole.ruleclass.IncapableOfWorkingEligibilityAndEntitlementRuleSet.impl.IncapableOfWorkingEligibilityUnitCalculator;
import curam.creole.ruleclass.IncapableOfWorkingEligibilityAndEntitlementRuleSet.impl.IncapableOfWorkingEligibilityUnitCalculator_Factory;
...
/**
 * Junit class for MOLSA Incapable Of Working
 * 
 * 
 */
@SuppressWarnings("restriction")
public class MOLSAIncapableOfWorkingEligibilityUnitCalculatorTest extends
MOLSACreoleBaseTest {
	public static final String MOLSA_RULESET = "IncapableOfWorkingEligibilityAndEntitlementRuleSet";
	 ...
	 *            default argument
	 */
	public MOLSAIncapableOfWorkingEligibilityUnitCalculatorTest(String arg0) {
		super(arg0);
		setRuleSetName();
	}
	...
	/**
	   ...
	   * Test Class : IncapableOfWorkingEligibilityUnitCalculator
	   */
	...
	public void testUnearnedIncomeAmount(){
		...
		final IncapableOfWorkingEligibilityUnitCalculator incapableOfWorkingObj = IncapableOfWorkingEligibilityUnitCalculator_Factory
		...
		incapableOfWorkingObj.caseID().specifyValue(caseParticipantRoleList.get(0).caseID().getValue());
		
		CREOLETestHelper.assertEquals(amountTimeline,
				incapableOfWorkingObj.unearnedIncomeAmount().getValue());
	}
	...
	public void testhasHouseholdPassedIncomeTestTimeline(){
		...
		.getFactory().newInstance(getSession(), caseID,
				saHouseholdUnitCalculator);
		incapableOfWorkingObj.caseID().specifyValue(caseID);
		incapableOfWorkingObj.totalCountableIncomeTimeline().specifyValue(amountTimeline);
		incapableOfWorkingObj.primaryBeneficiaryAmount().specifyValue(amount2Timeline);
		CREOLETestHelper.assertEquals(resultTimeline,
				incapableOfWorkingObj.hasHouseholdPassedIncomeTestTimeline().getValue());
	}
	...
	public void testtotalCountableIncomeTimeline(){
		...
		.getFactory().newInstance(getSession(), caseID,
				saHouseholdUnitCalculator);
		incapableOfWorkingObj.caseID().specifyValue(caseID);
		incapableOfWorkingObj.unearnedIncomeAmount().specifyValue(amountTimeline);
		incapableOfWorkingObj.totalExpenseAmountTimeline().specifyValue(amount2Timeline);
		CREOLETestHelper.assertEquals(amount3Timeline,
				incapableOfWorkingObj.totalCountableIncomeTimeline().getValue());
	}
	...
	public void testTotalExpenseAmountTimeline(){
		...
		CREOLETestHelper.assertEquals(amountTimeline,
				incapableOfWorkingObj.totalExpenseAmountTimeline().getValue());
	}
	...
	public void testMonthlyAmountTimeline(){
		...
		incapableOfWorkingObj.totalCountableIncomeTimeline()
				.specifyValue(amountTimeline);
		CREOLETestHelper.assertEquals(outputTimeline,
				incapableOfWorkingObj.monthlyAmountTimeline().getValue());
	}
	...
	public void testPrimaryBeneficiaryAmount(){
		...
		HeadOfHouse hohObj = HeadOfHouse_Factory.getFactory().newInstance(getSession());
		hohObj.caseID().specifyValue(caseID);
		hohObj.participant().specifyValue(caseParticipantRoleList.get(0).caseParticipantRoleID().getValue());
		
		incapableOfWorkingObj.caseID().specifyValue(caseID);
		incapableOfWorkingObj.eligibilityUnitMembers().specifyValue(caseParticipantRoleList);

		CREOLETestHelper.assertEquals(amountTimeline,
				incapableOfWorkingObj.primaryBeneficiaryAmount().getValue());
	}
	/**
	   * Test case for 'isMarriedTimeline' attribute. 
	   * Test Data: Marital status as married
	   * Test Class : IncapableOfWorkingCPRCalculator
	   */
	...
	public void testisMarriedTimeline(){
		...
		final IncapableOfWorkingCPRCalculator incapableOfWorkingCPRCalculatorObj = IncapableOfWorkingCPRCalculator_Factory
		.getFactory().newInstance(getSession(), caseParticipantRole);
			
		// Specifying marital status as Married
		...
		CREOLETestHelper.assertEquals(Timeline.TRUE_FOREVER,
				incapableOfWorkingCPRCalculatorObj.isMarriedTimeline().getValue());
	}
