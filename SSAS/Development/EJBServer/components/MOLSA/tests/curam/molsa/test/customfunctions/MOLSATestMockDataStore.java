...
import curam.core.sl.impl.RegisterProspectPerson;
...
import curam.util.transaction.DatabaseConnection;
...
//		rootEntity.addChildEntity(intakeProgram);
		...
		final EntityType secondPersonEntityType = datastore
				.getEntityType(IEGConstants.kPersonEntityName);
		final Entity secondPersonEntity = datastore.newEntity(secondPersonEntityType);
		...
		secondPersonEntity.setTypedAttribute("isMailingAddressSame", Boolean.TRUE);
		secondPersonEntity
				.setTypedAttribute("isMemberEnrolledInSchool", Boolean.TRUE);
		secondPersonEntity.setTypedAttribute("isPhysicallyChallenged", Boolean.FALSE);
		secondPersonEntity.setTypedAttribute("isPrimaryParticipant", Boolean.FALSE);
		...
		final EntityType secondPersonIncomeEntityType = datastore
				.getEntityType(MOLSADatastoreConst.kIncome);
		final Entity secondPersonIncome = datastore.newEntity(secondPersonIncomeEntityType);
		...
		secondPersonIncome.setTypedAttribute("startDate",
				Date.getCurrentDate().addDays(-100));
		secondPersonEntity.addChildEntity(secondPersonIncome);

		final EntityType expenseSecondPersonExpenseType = datastore.getEntityType("Expense");
		final Entity expenseSecondPerson = datastore.newEntity(expenseSecondPersonExpenseType);
		...
		expenseSecondPerson.setTypedAttribute("startDate",
				Date.getCurrentDate().addDays(-100));
		...
		relationShip.setTypedAttribute("relationshipType",RELATIONSHIPTYPECODEEntry.PARENT.getCode());
