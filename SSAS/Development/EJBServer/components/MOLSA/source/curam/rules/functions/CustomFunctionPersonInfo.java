  ...
  private void updateChildrenInfo(final IEG2Context ieg2Context)
    ...
    try {
      ...
      for (Entity personEntity : personEntities) {
        // Update Age
        personEntity.setAttribute(MOLSADatastoreConst.kCalculatedAge,String.valueOf(MOLSADateUtil
            .calculateAge(Date.getDate(personEntity
                .getAttribute(MOLSADatastoreConst.kDateOfBirth)))));

        if ((Integer.valueOf(personEntity
            .getAttribute(MOLSADatastoreConst.kCalculatedAge)).intValue()) > 60) {
          personEntity.setTypedAttribute(MOLSADatastoreConst.kIsSeniorCitizen,
              Boolean.TRUE);
        }
        if (!(personEntity
          ...
          if ((personEntity.getTypedAttribute(MOLSADatastoreConst.kGender)
              .equals(GENDEREntry.MALE.getCode()))
              && (Boolean) (personEntity
                  .getTypedAttribute(MOLSADatastoreConst.kIsMemberEnrolledInSchool))) {
            personEntity.setTypedAttribute(MOLSADatastoreConst.kIsChild,
                Boolean.TRUE);
            personEntity.update();
