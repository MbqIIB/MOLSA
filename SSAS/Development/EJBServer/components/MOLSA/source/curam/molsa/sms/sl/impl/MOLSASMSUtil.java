...
public class MOLSASMSUtil extends curam.molsa.sms.sl.base.MOLSASMSUtil {
	...
	@Override
	public void validateParticipantMobile() throws AppException,
			...
			if (authenticateSoapUser(soapUser)) {
				StringList concernRoleIDList = StringUtil
						.delimitedText2StringList(
								key.dtls.concernRoleTabbedList,
				...
				try {
					...
					for (String concernRoleID : concernRoleIDList) {
						...
						// sendResult.set
						if (!sendResult.getSendSmsResult().getResult()
								.equals("OK")) {
						...
						caseStatusConcernRoleIDICType.concernRoleID = Long
								.parseLong(concernRoleID);
						...
						long caseID = CaseHeaderFactory.newInstance()
								.searchICByStatusParticipantIDICType(
										caseStatusConcernRoleIDICType).dtls
								.get(0).caseID;
						smsDetails.caseID = caseID;

						smsDetails.subject = CodeTable.getOneItem(
								MOLSASMSMessageType.TABLENAME,
								CodeTable.getParentCode(
										MOLSASMSMESSAGETEMPLATE.TABLENAME,
										key.dtls.smsMessageType));
						ConcernRoleCommKeyOut commKeyOut = new MOLSACommunication()
								.createSMSReturningID(smsDetails);
						...
						molsasmsLogDtls.relatedID = commKeyOut.communicationID;
						...
						MOLSASMSLogFactory.newInstance()
								.insert(molsasmsLogDtls);
					}
	...
	public MOLSAParticipantDetailsList listParticipantByCriteria(
		...
		CuramValueList curamValueList = new CuramValueList(
				MOLSAParticipantDetails.class);
		try {
			curamValueList = DynamicDataAccess.executeNsMulti(
					MOLSAParticipantDetails.class, key.dtls, false, true,
					formatSQL(key.dtls).sqlStatement);
		} catch (ReadmultiMaxException e) {
			ValidationManagerFactory
					.getManager()
					.addInfoMgrExceptionWithLookup(
							new AppException(
									GENERAL.INF_GENERAL_SEARCH_TOO_MANY_RECORDS),
							"", InformationalElement.InformationalType.kError,
							"a", 3);
		...
		for (int i = 0; i < curamValueList.size(); i++) {
			...
			MOLSAParticipantDetails details = (MOLSAParticipantDetails) curamValueList
					.item(i);
			...
			details.addressString = ParticipantFactory.newInstance()
					.displaySingleLineAddress(otherAddressData).addressString;
			details.participantName = details.participantName.concat(" - ")
					.concat(details.qid);

			molsaParticipantDetailsList.dtls.dtls
					.addRef((MOLSAParticipantDetails) details);
		}
	...
	protected void validateSearchCriteria(
		...
		if (!(key.dtls.hasIncome) && !(key.dtls.isIncludeHouseHoldMembers)
				&& (key.dtls.age == CuramConst.gkZero)
				&& (key.dtls.caseStatus.length() == CuramConst.gkZero)
				&& (key.dtls.caseType.length() == CuramConst.gkZero)
				&& (key.dtls.educationLevel.length() == CuramConst.gkZero)
				&& (key.dtls.gender.length() == CuramConst.gkZero)
				&& (key.dtls.muncipality.length() == CuramConst.gkZero)
				&& (key.dtls.incomeFromDate.isZero())
				&& (key.dtls.incomeToDate.isZero())) {

			ValidationManagerFactory.getManager().throwWithLookup(
					new AppException(
							GENERALSEARCH.ERR_FV_SEARCH_CRITERIA_MISSING), "a",
					0);
	...
	protected SQLStatement formatSQL(
			curam.molsa.sms.facade.struct.MOLSAParticipantFilterCriteriaDetails dtls)
			throws AppException, InformationalException {
		...
		selectStrBuf
				.append("SELECT Distinct concernrole.primaryalternateid,concernrole.concernroleID,concernrole.concernRoleName, ");
		selectStrBuf
				.append("concernrole.primaryAddressID, person.dateofBirth ");
		...
		fromStrBuf
				.append("FROM concernrole, person, caseheader, caseparticipantrole, productdelivery ");
		...
		if (dtls.age > CuramConst.gkZero) {
			...
			whereStrBuf
					.append("person.dateofbirth <= :caluclatedDateFromAge AND ");
		}
		...
		if (dtls.muncipality.length() > CuramConst.gkZero) {
			...
			whereStrBuf
					.append("addresselement.elementvalue = :muncipality AND ");
			whereStrBuf
					.append("addresselement.addressID = address.addressID AND ");
			whereStrBuf
					.append("address.addressID = concernroleaddress.addressID AND ");
		}

		if (dtls.educationLevel.length() > CuramConst.gkZero) {
			fromStrBuf
					.append(", evidencetypedef as evddef, evidencedescriptor as evddes, dynamicevidencedataattribute, dynamicevidencedata ");
			whereStrBuf.append("evddef.evidencetypecode='DET0000517' AND ");
			whereStrBuf
					.append("dynamicevidencedataattribute.value= :educationLevel AND ");
			whereStrBuf
					.append("evddef.evidencetypecode=evddes.evidencetype AND ");
			whereStrBuf
					.append("evddes.participantid=concernrole.concernroleID AND ");
			whereStrBuf
					.append("evddes.statuscode in ('EDS1','EDS2','EDS3','EDS2001','EDS2007') AND ");
		}

		if (dtls.isIncludeHouseHoldMembers) {
			fromStrBuf
					.append(", evidencetypedef as edef, evidencedescriptor as edesc ");
			whereStrBuf.append("edef.evidencetypecode='DET0000256' AND ");
			whereStrBuf.append("edef.evidencetypecode=edesc.evidencetype AND ");
			whereStrBuf
					.append("edesc.participantid=concernrole.concernroleID AND ");
			whereStrBuf
					.append("edesc.statuscode in ('EDS1','EDS2','EDS3','EDS2001','EDS2007') AND ");
		} else {
			whereStrBuf
					.append("caseHeader.concernroleID=concernrole.concernroleID AND ");
		}

		if (dtls.hasIncome) {
			fromStrBuf.append(", evidencetypedef, evidencedescriptor ");
			whereStrBuf
					.append("evidencetypedef.evidencetypecode='DET0000514' AND ");
			whereStrBuf
					.append("evidencetypedef.evidencetypecode=evidencedescriptor.evidencetype AND ");
			whereStrBuf
					.append("evidencedescriptor.participantid=concernrole.concernroleID AND ");
			whereStrBuf
					.append("evidencedescriptor.statuscode in ('EDS1','EDS2','EDS3','EDS2001','EDS2007') AND ");
		}

		if (!(dtls.incomeFromDate.isZero())) {
			fromStrBuf.append(", instructionlineitem ");
			whereStrBuf
					.append("instructionlineitem.coverperiodfrom >= :incomeFromDate AND ");
			whereStrBuf
					.append("instructionlineitem.concernroleid=concernrole.concernroleid AND ");
			whereStrBuf
					.append("instructionlineitem.statuscode in ('ALL', 'REC', 'PRO', 'REV', 'TRF') AND ");
			whereStrBuf
					.append("instructionlineitem.creditdebittype in ('CDT') AND ");
		...
		if (!(dtls.incomeToDate.isZero())) {
			...
			whereStrBuf
					.append("ilitem.concernroleid=concernrole.concernroleid AND ");
			whereStrBuf
					.append("ilitem.statuscode in ('ALL', 'REC', 'PRO', 'REV', 'TRF') AND ");
			whereStrBuf.append("ilitem.creditdebittype in ('CDT') AND ");
		}

		whereStrBuf
				.append("person.concernroleID=concernrole.concernroleID AND ");
		whereStrBuf
				.append("caseparticipantrole.participantroleID=concernrole.concernroleID ");
		whereStrBuf.append("Group By concernrole.concernroleID");

		sqlStatement.sqlStatement = selectStrBuf.toString()
				+ intoStrBuf.toString() + fromStrBuf.toString()
				+ whereStrBuf.toString();
		return sqlStatement;
	...
	public MOLSAAdditionalBenefitDetailsList listParticipantAdditionalBenefits(
			curam.citizenaccount.facade.struct.ConcernRoleKey key)
			throws AppException, InformationalException {
		...
		EvidenceDescriptorDtlsList descriptorDtlsList = descriptor
				.searchByParticipantIDEvidenceType(paramKey);
		...
		for (EvidenceDescriptorDtls descriptorDtls : descriptorDtlsList.dtls) {
			...
			additionalBenefitDetails.participantName = concernRoleObj
					.readConcernRoleName(concernRoleKey).concernRoleName;

			String receivedDate = dynamicEvidenceDataDetails
					.getAttribute(KDate).getValue();

			String amount = dynamicEvidenceDataDetails.getAttribute(KAmount)
					.getValue();

			additionalBenefitDetails.amount = amount + " "
					+ Configuration.getProperty("curam.financial.basecurrency");
			...
			additionalBenefitDetails.receivedDate = curam.util.type.Date
					.getFromJavaUtilDate(receivedDate1);

			additionalBenefitDetailsList.dtls.dtls
					.addRef(additionalBenefitDetails);
	...
	public void exportParticipantsToExcel(
			MOLSAConcernRoleListAndMessageTextDetails key) throws AppException,
			InformationalException {
		...
		if (key.dtls.concernRoleTabbedList.length() == 0) {
			...
			ValidationManagerFactory.getManager().throwWithLookup(
					new AppException(MOLSASMSSERVICE.NO_PARTICIPANTS_SELECTED),
					"a", 0);
		...
		for (int i = 0; i < concernRoleID.length; i++) {
			...
			CuramValueList curamValueList = new CuramValueList(
					MOLSAParticipantDetails.class);
			curamValueList = DynamicDataAccess.executeNsMulti(
					MOLSAParticipantDetails.class, concernRoleKey, false, true,
			...
			for (int x = 0; x < curamValueList.size(); x++) {
				...
				Address addressObj = AddressFactory.newInstance();
				AddressKey paramAddressKey = new AddressKey();
				paramAddressKey.addressID = Long
						.parseLong(details.addressString);
				AddressDtls addressDtls = addressObj.read(paramAddressKey);
				OtherAddressData otherAddressData = new OtherAddressData();
				otherAddressData.addressData = addressDtls.addressData;
				details.addressString = ParticipantFactory.newInstance()
	...
	protected SQLStatement getParticipantDetailsQuery(
		...
		selectStrBuf
				.append("SELECT Distinct concernrole.primaryalternateid, concernrole.concernRoleName, ");
		selectStrBuf
				.append("concernrole.primaryAddressID, person.dateofBirth ");
