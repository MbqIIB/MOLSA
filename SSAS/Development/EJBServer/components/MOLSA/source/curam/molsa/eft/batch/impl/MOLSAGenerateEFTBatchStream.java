 ...
 * 
 */
public class MOLSAGenerateEFTBatchStream extends curam.molsa.eft.batch.base.MOLSAGenerateEFTBatchStream {
  ...
  public void process(BatchProcessStreamKey batchProcessStreamKey) throws AppException, InformationalException {
    ...
    MOLSAGenerateEFTBatchStreamWrapper  molsaGenerateEFTBatchStreamWrapper = new MOLSAGenerateEFTBatchStreamWrapper(this);
    SecurityImplementationFactory.register();
    if (batchProcessStreamKey.instanceID.length() == 0) {
      batchProcessStreamKey.instanceID = BATCHPROCESSNAME.MOLSA_GENERATE_EFT;
    }
    batchStreamHelper.runStream(batchProcessStreamKey, molsaGenerateEFTBatchStreamWrapper);
  ...
  /**
   * Invoked from the batch program to generate the EFT documents.
   * Read all the processed Payment Instrument and create the EFT documents (Word and Exel).
   * Once the exel is generated, the Payment Instrument Status is updated to Issued.
   * 
  ...
  public BatchProcessingSkippedRecord processRecord(
      BatchProcessingID batchProcessingID, MOLSAGenerateEFTParam generateEFTParam) 
    ...
    PaymentInstrumentDtlsList paymentInstrumentDtlsList = paymentInstrumentObj.searchByStatusCode(piStatusCode);
    
     
    
    BatchProcessingSkippedRecord batchProcessingSkippedRecord = new BatchProcessingSkippedRecord();

    MOLSAGenerateEFTDetailList bankGenerateEFTDetailList = generateExelForBank(paymentInstrumentDtlsList, generateEFTParam);   
   ...
   *           General ExceptionList
   */
  private void generateMsWord(double totalAmount) 
  throws AppException, InformationalException {

    MOLSAGenerateEFTMsWordDetail generateEFTMsWordDetail = new MOLSAGenerateEFTMsWordDetail();
    String compBankAccountID = Configuration.getProperty(EnvVars.EFT_BANKACCOUNTID);
    ...
    generateEFTMsWordDetail.socialAffairMinisterName = 
      Configuration.getProperty(EnvVars.EFT_NAME_OF_ASST_MINISTER_SOCIAL_AFFAIR);
    generateEFTMsWordDetail.securityDirectorName = 
      Configuration.getProperty(EnvVars.EFT_NAME_OF_SOCIAL_SECURITY_DIRECTOR);
    
    String dayOfMonth = Configuration.getProperty(EnvVars.ENV_MOLSA_FINANCIAL_PAYMENT_DAY);
    ...
    generateEFTMsWordDetail.transferAmount=totalAmount+" / "+Configuration.getProperty(EnvVars.ENV_BASECURRENCY);
  ...
  /**
   * Checks whether the Payment is generated for the Financial Component.
   * This also remove the Benefit UnderPayment.
   * 
   * @param financialComponentDtls FinancialComponentDtls
   * @return boolean
   * @throws AppException
   *           General Exception
   * @throws InformationalException
   *           General ExceptionList
   */
  private boolean checkPaymentIsGenerated(FinancialComponentDtls financialComponentDtls)  
  throws AppException, InformationalException { 
    ...
    Count count = instructionLineItemObj.countByFinancialCompID(financialComponentID);
   ...
   * 
   * @param financialComponentDtlsList FinancialComponentDtlsList
   ...
   *           General ExceptionList
   */
  private FinancialComponentDtlsList filterFinancialComponent(FinancialComponentDtlsList financialComponentDtlsList)  
  throws AppException, InformationalException { 
    for(FinancialComponentDtls  financialComponentDtls : financialComponentDtlsList.dtls.items() ) {
  ...
  //Suspended Cases Amount
    ...
    CaseHeaderDtlsList caseHeaderDtlsList = caseHeaderObj.searchByStatusCode(caseHeaderByStatusKey);
    ...
    for(CaseHeaderDtls caseHeaderDtls :caseHeaderDtlsList.dtls.items()) {
      ...
      caseStatusDtls = caseStatusObj.readCurrentStatusByCaseID1(currentCaseStatusKey);
      ...
      FinancialComponentDtlsList financialComponentDtlsList = financialComponentObj.searchByStatusCaseID(fcstatusCodeCaseID);
      ...
      if(financialComponentDtls != null ) {
        ...
        generateEFTDetail.deptCode= Configuration.getProperty(EnvVars.EFT_DEPT_CODE);
        generateEFTDetail.staffNumber= MOLSAParticipantHelper.returnConcernRoleAlternateID(financialComponentDtls.concernRoleID, 
        ...
        long concernRoleID = MOLSAParticipantHelper.returnConcernRoleIDFromCaseParticipantRoleID(caseNomineeDtls.caseParticipantRoleID);
        
        generateEFTDetail.fullname_ar = MOLSAParticipantHelper.returnConcernRoleName(concernRoleID);
        generateEFTDetail.fullname_en = MOLSAParticipantHelper.returnAlternateName(concernRoleID, ALTERNATENAMETYPE.ENGLISH);
        
        generateEFTDetail.currencyCode= Configuration.getProperty(EnvVars.ENV_BASECURRENCY);
        
        
        frequencyPattern = new FrequencyPattern(financialComponentDtls.frequency);
        Date[] dates = frequencyPattern.getAllOccurrences(financialComponentDtls.dueDate, currentDate);
        double amount = financialComponentDtls.amount.getValue() * dates.length;
   ...
   *           General ExceptionList
   */
  private FinancialComponentDtls returnLastFinancialComponentForSuspendedCase(FinancialComponentDtlsList financialComponentDtlsList)
  throws AppException, InformationalException { 
    ...
    Collections.sort(financialComponentDtlsList.dtls, new Comparator<FinancialComponentDtls>() {
      public int compare(FinancialComponentDtls o1, FinancialComponentDtls o2) {
    ...
    for(FinancialComponentDtls  financialComponentDtls : financialComponentDtlsList.dtls.items() ) {
      if(!financialComponentDtls.expiryDate.equals(Date.kZeroDate) 
          && (
              financialComponentDtls.nextProcessingDate.before(Date.getCurrentDate()) 
              || financialComponentDtls.nextProcessingDate.equals(Date.getCurrentDate())
              )){
   ...
   * 
   * @param paymentInstrumentDtlsList PaymentInstrumentDtlsList
   * @throws AppException
   *           General Exception
   * @throws InformationalException
   *           General ExceptionList
   */
  private void updatePaymentInstrumentStatus(PaymentInstrumentDtlsList paymentInstrumentDtlsList) 
  throws AppException, InformationalException {
    PaymentInstrument paymentInstrumentObj = PaymentInstrumentFactory.newInstance();
    EffectiveDateReconcilStatusVersionNo effectiveDateReconcilStatusVersionNo ;
    PaymentInstrumentKey paymentInstrumentKey ;
    for (PaymentInstrumentDtls paymentInstrumentDtls : paymentInstrumentDtlsList.dtls.items()) {
      ...
      effectiveDateReconcilStatusVersionNo.effectiveDate= Date.getCurrentDate();
      ...
      paymentInstrumentObj.modifyEffectiveDateReconcilStatus(paymentInstrumentKey, effectiveDateReconcilStatusVersionNo);
    }
  }
   ...
   *           General ExceptionList
   */
  private MonthYearDetails populateExelGenericDetails(MOLSAGenerateEFTDetailList generateEFTDetailList)  
  throws AppException, InformationalException {
    String compBankAccountID = Configuration.getProperty(EnvVars.EFT_BANKACCOUNTID);
    BankAccountDtls bankAccountDtls = 
      MOLSAFinancialHelper.returnBankAccountDetails(Long.parseLong(compBankAccountID));
    ...
    generateEFTDetailList.compCode = Configuration.getProperty(EnvVars.EFT_COMP_CODE);
    generateEFTDetailList.fileDesc = Configuration.getProperty(EnvVars.EFT_SAL_CODE);
  ...
  private void generateExelForFinance(MOLSAGenerateEFTDetailList generateEFTDetailList, 
  ...
  throws AppException, InformationalException {
    ...
    LocalisableString remarks = new LocalisableString(MOLSABPOGENERATEEFT.REMARKS_CONTENT);
    ...
    MOLSAGenerateEFTHelper.newInstance().generateExel(generateEFTDetailList, 
        generateEFTParam, MOLSAGenerateEFTHelper.getExelName(false, monthYearDetails));
   ...
   *           General ExceptionList
   */
  private MOLSAGenerateEFTDetailList generateExelForBank(PaymentInstrumentDtlsList paymentInstrumentDtlsList, 
      MOLSAGenerateEFTParam generateEFTParam) 
  throws AppException, InformationalException {
    ...
    for (PaymentInstrumentDtls paymentInstrumentDtls : paymentInstrumentDtlsList.dtls.items()) {
      generateEFTDetail =  new MOLSAGenerateEFTDetail();
      generateEFTDetail.deptCode= Configuration.getProperty(EnvVars.EFT_DEPT_CODE);
      generateEFTDetail.staffNumber= MOLSAParticipantHelper.returnConcernRoleAlternateID(paymentInstrumentDtls.concernRoleID, 
          CONCERNROLEALTERNATEID.INSURANCENUMBER);
      
      generateEFTDetail.fullname_ar = MOLSAParticipantHelper.returnConcernRoleName(paymentInstrumentDtls.concernRoleID);
      generateEFTDetail.fullname_en = MOLSAParticipantHelper.returnAlternateName(paymentInstrumentDtls.concernRoleID, ALTERNATENAMETYPE.ENGLISH);
      
      bankAccountDtls = 
        MOLSAFinancialHelper.returnBankAccountDetails(paymentInstrumentDtls.bankAccountID);
    ...
    MOLSAGenerateEFTHelper.newInstance().generateExel(generateEFTDetailList, 
        generateEFTParam, MOLSAGenerateEFTHelper.getExelName(true, monthYearDetails));
    return generateEFTDetailList;
