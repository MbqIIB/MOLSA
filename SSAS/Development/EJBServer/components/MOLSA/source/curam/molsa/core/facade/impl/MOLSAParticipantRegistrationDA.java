...
/**
 * The Class for participant Registration.
 * This class is overridden from OOTB
 */
public abstract class MOLSAParticipantRegistrationDA extends curam.molsa.core.facade.base.MOLSAParticipantRegistrationDA {
   ...
   *           General Exception
   */
  public ParticipantRegistrationWizardResult setRegisterPersonForPDCDetails(MOLSAPersonRegistrationDetails molsaRegistrationDtls, WizardStateID stateID, ActionIDProperty actionID)
      throws AppException, InformationalException {

    PersonRegistrationDetails registrationDtls = molsaRegistrationDtls.dtls;
    if(!(registrationDtls.phoneNumber.isEmpty())){
    	if(!((registrationDtls.phoneNumber.length() == 8) && (registrationDtls.phoneNumber.matches("[0-9]+"))))
        {
          AppException e = new AppException(MOLSANOTIFICATION.PHONE_NO);
          throw e;
        }
    }
    ParticipantRegistrationWizardResult participantRegistrationWizardResult = super.setRegisterPersonForPDCDetails(registrationDtls, stateID, actionID);
   ...
   *           Generic Exception Signature.
   */
  public RegisterPersonWizardSearchDetails setRegisterPersonSearchCriteriaDetails(final PersonSearchWizardKey personSearchWizardKey, final WizardStateID wizardStateID,
      final ActionIDProperty actionIDProperty) throws AppException, InformationalException {

    if(personSearchWizardKey.searchKey.referenceNumber.isEmpty() || !personSearchWizardKey.searchKey.referenceNumber.matches("[0-9]+") ||  MOLSAConstants.gkEleven != personSearchWizardKey.searchKey.referenceNumber.length() )
    {
  ...
  //check for the action button use has clicked
    if (CuramConst.kNextPageAction.equalsIgnoreCase(actionIDProperty.actionIDProperty)) {
      RegisterPersonState registerPersonState = (RegisterPersonState) wizardPersistentState.read(wizardStateID.wizardStateID);
      ...
      registerPersonState.registrationDtls = molsaParticipantHelper.getMOIDetailsByQID(qid);
      registerPersonState.searchKey = personSearchWizardKey;
      registerPersonState.searchKey.stateID = wizardStateID;
      wizardPersistentState.modify(wizardStateID.wizardStateID, registerPersonState);
    } else if (CuramConst.kSearchAction.equalsIgnoreCase(actionIDProperty.actionIDProperty)) {
      registerPersonWizardSearchResult.searchResult = searchPersonDetails(personSearchWizardKey);
    } else if (CuramConst.kResetAction.equalsIgnoreCase(actionIDProperty.actionIDProperty)) {
      RegisterPersonState registerPersonState = (RegisterPersonState) wizardPersistentState.read(wizardStateID.wizardStateID);
