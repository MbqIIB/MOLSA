...
public class MOLSABulkPDCGeneratorBatch extends curam.molsa.pdc.generator.base.MOLSABulkPDCGeneratorBatch {
  ...
  public void process() throws AppException, InformationalException {
    ...
    try {
      ...
      Trace.kTopLevelLogger.info("********  MM Bulk PDC Generation Chunker. Total Cases with Legacy Benefit evidences and Benefit Evidence of type ELE is ==> "
          + curamValueList.size());
    } catch (Exception e) {
      Trace.kTopLevelLogger.info("********  MM Bulk PDC Generation Chunker. Couldn't read the IC cases to be procesed. See error below ******");
      e.printStackTrace();
    }
    ...
    Trace.kTopLevelLogger.info("********  MM Bulk PDC Generation Chunker. Total no of IC cases being processed ****** " + curamValueList.size());
    ...
    batchStreamHelper.runChunkMain(BATCHPROCESSNAME.MOLSA_BULKPDC, null, molsaBulkPDCGeneratorBatchWrapper, batchProcessingIDList, chunkMainParameters,
