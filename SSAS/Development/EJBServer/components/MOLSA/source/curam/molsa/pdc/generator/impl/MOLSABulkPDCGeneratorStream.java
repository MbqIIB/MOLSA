...
public class MOLSABulkPDCGeneratorStream extends curam.molsa.pdc.generator.base.MOLSABulkPDCGeneratorStream {
  ...
  public BatchProcessingSkippedRecord processRecord(BatchProcessingID batchProcessingID, MOLSAMoiDtls MOLSAMoiDtls) throws AppException, InformationalException {
    Trace.kTopLevelLogger.info("Processing caseID ==> " + batchProcessingID.recordID);
    ...
    try {
       ...
       * }
       */
      CaseReferenceProductNameConcernRoleName caseRefProductNameConcernRoleName = MaintainCaseFactory.newInstance().readCaseReferenceConcernRoleNameProductNameByCaseID(caseIDKey);

      System.out.println("caseRefProductNameConcernRoleName -->" + caseRefProductNameConcernRoleName.concernRoleName);
      Trace.kTopLevelLogger.info("********  Processing caseID Successful ==> " + batchProcessingID.recordID);
      ...
      /*
       * try { MMBatchProcessFailureDtls mmBatchProcessFailureDtls = new
       * MMBatchProcessFailureDtls();
       * mmBatchProcessFailureDtls.mmBatchProcessFailureID =
       * curam.util.type.UniqueID.nextUniqueID("MMBATFAIL");
       * mmBatchProcessFailureDtls.batchProcessName =
       * BATCHPROCESSNAME.BULK_PDC_GENERATION;
       * mmBatchProcessFailureDtls.mmFailedRecordID =
       * batchProcessingID.recordID;
       * MMBatchProcessFailureFactory.newInstance().insert(
       * mmBatchProcessFailureDtls); } catch (Exception e) {
