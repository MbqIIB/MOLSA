...
public class MOLSAApplicationImpl extends ApplicationImpl {
  ...
  /**
   * Creates and populates the person entity on the data store for a prospect or
   * registered person on the Intake.
   * 
   * @param dataStore
  ...
  protected final void createMOLSADatastorePersonEntity(
    ...
    final Entity intakeApplicaton = dataStore.newEntity(intakeApplicatonType);
    ...
    for (int j = 0; j < addressMapList.dtls.size(); j++) {
      ...
      if (!(addressMapList.dtls.item(j).value.length() == 0)) {
        ...
        if (addressMapList.dtls.item(j).name
          ...
          personAddressEntity.setTypedAttribute(MOLSADatastoreConst.kPostCode,
              postCode);
    ...
    if (null != addressList && addressList.size() > 1) {
      for (Address mailAddress : addressList) {
        ...
        if (mailAddress.getAddressType(concernRole).toString()
          ...
          mailAddressDataString.addressData = mailAddress.getAddressData();
            ...
            for (int j = 0; j < mailingAddressMapList.dtls.size(); j++) {
              if (!(mailingAddressMapList.dtls.item(j).value.length() == 0)) {
                ...
                if (mailingAddressMapList.dtls.item(j).name
                  ...
                  personMailingAddressEntity.setTypedAttribute(
                      MOLSADatastoreConst.kMailingStreet, street);
                } else if (mailingAddressMapList.dtls.item(j).name
                    .equals(curam.codetable.ADDRESSELEMENTTYPE.UNIT_NUMBER)) {
                  String buildingNumber = mailingAddressMapList.dtls.item(j).value;
                  personMailingAddressEntity.setTypedAttribute(
                      MOLSADatastoreConst.kMailingBuildingNumber,
                      buildingNumber);
                } else if (mailingAddressMapList.dtls.item(j).name
                    .equals(curam.codetable.ADDRESSELEMENTTYPE.LINE4)) {
                  String buildingType = mailingAddressMapList.dtls.item(j).value;
                  personMailingAddressEntity.setTypedAttribute(
                      MOLSADatastoreConst.kMailingBuildingType, buildingType);

                } else if (mailingAddressMapList.dtls.item(j).name
                    .equals(curam.codetable.ADDRESSELEMENTTYPE.ZIP)) {
                  String postCode = mailingAddressMapList.dtls.item(j).value;
                  personMailingAddressEntity.setTypedAttribute(
                      MOLSADatastoreConst.kMailingPostCode, postCode);

                } else if (mailingAddressMapList.dtls.item(j).name
                    .equals(curam.codetable.ADDRESSELEMENTTYPE.LINE5)) {
                  String electricityNumber = mailingAddressMapList.dtls.item(j).value;
                  personMailingAddressEntity.setTypedAttribute(
                    ...
                    .equals(curam.codetable.ADDRESSELEMENTTYPE.POBOXNO)) {
                  String poBox = mailingAddressMapList.dtls.item(j).value;
                  personMailingAddressEntity.setTypedAttribute(
                      MOLSADatastoreConst.kMailingPOBox, poBox);

                } else if (mailingAddressMapList.dtls.item(j).name
                    .equals(curam.codetable.ADDRESSELEMENTTYPE.COUNTRY)) {
                  String country = mailingAddressMapList.dtls.item(j).value;
                  personMailingAddressEntity.setTypedAttribute(
                      MOLSADatastoreConst.kMailingCountry, country);
    ...
    try {
      ...
      for (final PDCEvidenceDetails pdcEvidenceDetails : pdcEvidenceList.list) {
        ...
        if (pdcEvidenceDetails.evidenceType
          ...
          final String nameType = dynamicEvidenceDataDetails.getAttribute(
              MOLSADatastoreConst.kPDCNamesNameType).getValue();
          if (nameType.equals(ALTERNATENAMETYPEEntry.REGISTERED.getCode())) {
            final String firstName = dynamicEvidenceDataDetails.getAttribute(
                MOLSADatastoreConst.kPDCNamesFirstName).getValue();
            final String middleInitial = dynamicEvidenceDataDetails
                .getAttribute(MOLSADatastoreConst.kPDCNamesMiddleName)
                .getValue();
            final String lastName = dynamicEvidenceDataDetails.getAttribute(
                MOLSADatastoreConst.kPDCNamesLastName).getValue();
            personEntity.setTypedAttribute(MOLSADatastoreConst.kFirstName,
                firstName);
            personEntity.setTypedAttribute(MOLSADatastoreConst.kMiddleInitial,
                middleInitial);
            personEntity.setTypedAttribute(MOLSADatastoreConst.kLastName,
                lastName);
          }
          ...
          final String dateOfBirth = dynamicEvidenceDataDetails.getAttribute(
              MOLSADatastoreConst.kPDCBirthAndDeathDateOfBirth).getValue();
          // Set the data store attributes by Prospect Person.
          personEntity.setTypedAttribute(MOLSADatastoreConst.kDateOfBirth,
              Date.fromISO8601(dateOfBirth));
        } else if (pdcEvidenceDetails.evidenceType
            .equals(MOLSADatastoreConst.kPDCIdentificationEvidence)) {
          // check ssn matches
          final String altIDType = dynamicEvidenceDataDetails.getAttribute(
          ...
          if (altIDType.equals(mappingAlternateIDType)) {
            final String alternateID = dynamicEvidenceDataDetails.getAttribute(
                MOLSADatastoreConst.kPDCIdentificationsAlternateID).getValue();
            personEntity.setTypedAttribute(MOLSADatastoreConst.qidNumber,
                alternateID);
          ...
          final String gender = dynamicEvidenceDataDetails.getAttribute(
              MOLSADatastoreConst.kPDCGenderGender).getValue();
          personEntity.setTypedAttribute(MOLSADatastoreConst.kGender, gender);
        } else if (pdcEvidenceDetails.evidenceType
          ...
          personEntity.setTypedAttribute(MOLSADatastoreConst.kEmailAddress,
              emailAddress);
        } else if (pdcEvidenceDetails.evidenceType
            .equals(MOLSADatastoreConst.kPDCContactPreferences)) {
          preferredLanguage = dynamicEvidenceDataDetails.getAttribute(
              MOLSADatastoreConst.kPreferredLanguage).getValue();
          preferredCommunicationMethod = dynamicEvidenceDataDetails
              .getAttribute(MOLSADatastoreConst.kPreferredCommunication)
              .getValue();
        } else if (pdcEvidenceDetails.evidenceType
            .equals(MOLSADatastoreConst.kPDCPhoneNumber)) {
          primaryPhoneNumberID = Long.valueOf(dynamicEvidenceDataDetails
              .getAttribute(MOLSADatastoreConst.kPhoneNumber).getValue());
        }
    ...
    intakeApplicaton.setTypedAttribute(MOLSADatastoreConst.kDateOfApplication,
        Date.getCurrentDate());
    ...
    if (!StringHelper.isEmpty(preferredLanguage)) {
      ...
      contactPreferences.setAttribute(ApplicationConst.kPreferredLanguage,
          preferredLanguage);
    }
    ...
    if (!StringHelper.isEmpty(personMailingAddressEntity
      ...
      personEntity.setTypedAttribute(MOLSADatastoreConst.kIsMailingAddressSame,
          IEGYESNOEntry.NO);
    } else {
      personEntity.setTypedAttribute(MOLSADatastoreConst.kIsMailingAddressSame,
          IEGYESNOEntry.YES);
  ...
  private void populateDatastoreWithAlternateID(Entity personEntity,
    ...
    for (final AlternateIDRMDtls details : result.details.dtls) {
      ...
      if (details.typeCode.equals(alternateIDType)
          && details.statusCode.equals(curam.codetable.RECORDSTATUS.NORMAL)
          && (details.endDate.isZero() || !details.endDate.before(Date
              .getCurrentDate()))
  ...
  private Entity populateDataStorePhoneNumber(final Datastore datastore,
    ...
    if (typeCode.equals(PHONETYPEEntry.PERSONAL.getCode())) {
      contactDetails.setTypedAttribute(MOLSADatastoreConst.kHomePhoneNumber,
          phoneNumber);
      entry = PHONETYPEEntry.PERSONAL;

    } else if (typeCode.equals(PHONETYPEEntry.BUSINESS.getCode())) {
      contactDetails.setTypedAttribute(MOLSADatastoreConst.kWorkPhoneNumber,
          phoneNumber);
      ...
      contactDetails.setTypedAttribute(MOLSADatastoreConst.kOtherPhoneNumber,
          phoneNumber);
  ...
  public void createAndStoreApplicationPDF() throws AppException,
    ...
    cmsMetadata.add(CMSMetadataConst.kCaseID, Long.toString(getCase().getID()));
   ...
   *           InformationalException
   */
  private long createReportAttachment(final ByteArrayOutputStream outputStream,
    ...
    attachmentDtls.attachmentStatus = ATTACHMENTSTATUSEntry.ACTIVE.getCode();
  ...
  /**
   * Reads the report data from the data store and converts it to XML. The data
   * is returned based on the given locale. An exception is thrown if the schema
   * cannot be read.
   * 
  ...
  private String getReportXMLFromDatastore() throws AppException {
    ...
    final DocumentFormatter docFormatter = new DocumentFormatter(false, true);
  ...
  private void addIntakePrograms(final List<PROGRAMTYPEEntry> programs,
    ...
    /*
     * Only insert the programs if the entity exists and program types have been
     * selected
     */
  ...
  public long start(final APPLICATIONTYPEEntry applicationType,
    ...
    iegScriptexecutionIdentifier = iegRuntime
        .createScriptExecutionExistingRootEntity(iegScriptID, iegSchemaname,
            iegRootEntityID);
    ...
    applicationStatus.setUserName(userDAO.get(TransactionInfo.getProgramUser())
        .getUsername());
    ...
    for (PROGRAMTYPEEntry programType : programs) {
      ...
      programApplication.setProgramRequestDate(getFilingDate().getDateTime());
