			...
			<!-- BEGIN, CR00356535, PB -->
              ...
              <fo:block space-before.optimum="1pt" space-after.optimum="2pt">
                <fo:block text-align="justify">
                  ...
                  <fo:inline font-size="12pt"><xsl:apply-templates select="FIELD[FNAME='providerGroupName']"/> agrees that the following provider(s)
                    will deliver the services listed below from <xsl:apply-templates select="FIELD[FNAME='startDate']"/> to 
                    <xsl:apply-templates select="FIELD[FNAME='endDate']"/> for a fixed amount of <xsl:apply-templates select="FIELD[FNAME='totalContractAmount']"/>. 
                  </fo:inline>
                  <fo:block>
                    <fo:leader leader-pattern="space"/>
                  </fo:block>
