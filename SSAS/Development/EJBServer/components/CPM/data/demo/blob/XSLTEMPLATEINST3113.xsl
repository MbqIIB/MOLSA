              ...
              <fo:block space-before.optimum="1pt" space-after.optimum="2pt">
                <fo:block text-align="justify">
                  ...
                  <fo:inline font-size="12pt">
                    We wish to inform you that roster line item
                    <xsl:apply-templates select="FIELD[FNAME='rliReferenceNumber']" />
                    from attendance roster
                    <xsl:apply-templates select="FIELD[FNAME='rosterReferenceNumber']" />
                    submitted on
                    <xsl:apply-templates select="FIELD[FNAME='dateSubmitted']" />
                    for the delivery of
                    <xsl:apply-templates select="FIELD[FNAME='serviceOfferingName']" />
                    was denied for the following reason:
                    <xsl:apply-templates select="FIELD[FNAME='denialReason']" />
                    .
                  </fo:inline>
                  <fo:block>
                    <fo:leader leader-pattern="space" />
