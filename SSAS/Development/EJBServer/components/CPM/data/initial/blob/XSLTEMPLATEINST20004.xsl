...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
      ...
      <fo:page-sequence master-reference="only">
        ...
        <fo:flow flow-name="xsl-region-body">
          ...
          <xsl:variable name="caseWorkerEmail"><xsl:value-of select="FIELD[FNAME='caseWorkerEmail']/VALUE"/></xsl:variable>
          <xsl:variable name="caseWorkerPhoneNumber"><xsl:value-of select="FIELD[FNAME='caseWorkerPhoneNumber']/VALUE"/></xsl:variable>
          ...
          <fo:block xsl:use-attribute-sets="Normal_3"> Dear
            <xsl:apply-templates select="FIELD[FNAME='providerDetails']/STRUCT[SNAME='ProviderNotificationDetails']/FIELD[FNAME='providerName']"/>, </fo:block>
         ...
         <xsl:if
            test="string-length($caseWorkerEmail) > 0 and string-length($caseWorkerPhoneNumber) > 0">
          <fo:block xsl:use-attribute-sets="Normal_4">
           If you have any questions please contact me on 
            <xsl:apply-templates select="FIELD[FNAME='caseWorkerPhoneNumber']"/> or 
  ...
  <xsl:template match="FIELD[FNAME='clientDetailsList']/STRUCT[SNAME='ClientNotificationDetailsList']/FIELD[FNAME='clientDetails']/STRUCT_LIST">
    ...
    <xsl:for-each select="STRUCT[SNAME='ClientNotificationDetails']">
      ...
      <xsl:variable name="address"><xsl:value-of select="FIELD[FNAME='address']/VALUE"/></xsl:variable>
      <xsl:variable name="phoneNumber"><xsl:value-of select="FIELD[FNAME='phoneNumber']/VALUE"/></xsl:variable>

        <fo:block xsl:use-attribute-sets="Normal_6">  <xsl:apply-templates
            select="FIELD[FNAME='name']"/>
        <!-- END, CR00356535 -->
        <xsl:if
          test="string-length($address) > 0 "> 
             with an address of <xsl:apply-templates
               select="FIELD[FNAME='address']"/>
        </xsl:if>
          <xsl:if
            test="string-length($phoneNumber) > 0 "> 
            and telephone number <xsl:apply-templates
              select="FIELD[FNAME='phoneNumber']"/>
          </xsl:if>.
          </fo:block>
