...
=====================================================*/
{
  ...
  var header = t.getElementsByTagName("th");
  ...
  for(var i=0; i<n; i++)
  {
    ...
    if (header[i].getAttribute("type")=="text") {
      if(f==undefined || f["col_"+i]==undefined || f["col_"+i]=="none")
      {
        ...
        document.getElementById("flt"+i+"_"+id).className = "flt";
...
=====================================================*/
{
  ...
  for(var k=start_row; k<row.length; k++)
  {
    ...
    if(nchilds == ncells){// checks if row has exact cell #
      ...
      {
        if (header[t].getAttribute("type")=="text")
        {
