...
require(["idx/oneui/form/FilteringSelect", 
...
IEGAJAXCall.prototype.fillCTWithBlank = function (theResult, placeholderText) {
  ...
  selectOptions[0] = { value:"", label:placeholderText };
  ...
  dijit.byId( this.target.id ).destroy( true );
  ...
  // create the widget
  var select = new idx.oneui.form.FilteringSelect({
    name: this.target.id,
    store: os,          // list of items for the dropdown
    style: "width: 100%",
    searchAttr:'label', // this is needed by a FilteringSelect
    maxHeight: -1
    },
